...
This section visually maps the technical architecture into academic models. All Data Flow Diagrams (DFDs) are explicitly rendered using **Gane & Sarson** notation (utilizing rounded rectangles for processes, squares for external entities, and database cylinders representing data stores).

### 4.1 Context Diagram (Gane & Sarson)
The Context Diagram treats the entire platform as a single process interacting with external entities.
...
Visualizes entities and relationships using academic Chen's notation (Diamonds = Relationships).
...
### 4.4 DFD Level 0 (Gane & Sarson)
Level 0 breaks the main system (Process 0.0) down into its primary subsystems and introduces Data Stores.
...
flowchart TB
    ...
    %% Gane & Sarson Processes (Rounded Rectangles)
...
### 4.5 DFD Level 1 (Gane & Sarson)
Level 1 decomposes Subsystem 2.0 (Manage Projects) into its detailed operational processes.
...
flowchart TB
    ...
    %% Gane & Sarson Processes
    P21([2.1 Create Project])
    ...
    %% Data Flows
    C -- "Project Details, Budget" --> P21
    P21 -- "Save Pending Project" --> D2
    
    D -- "Read Pending Projects" --> D2
    D -- "Apply Request" --> P22
    P22 -- "Save Application" --> D3

    C -- "Read Applications" --> D3
    C -- "Accept / Reject" --> P23
    P23 -- "Update App Status" --> D3
    P23 -- "Activate Project" --> D2
```
